...
const Code = ({ queryHome }) => {
  ...
  const { sendJsonMessage, lastJsonMessage, readyState } = useWebSocket(
    `wss://notanyai-backend.onrender.com/wss/chat?token=${token}`
  );
  const [gotmsg, setgotmsg] = useState(false);
  const instanceRef = useRef(null);
  const [webcontainerInstance, setWebcontainerInstance] = useState(null);

  // Boot WebContainer immediately on mount for faster loading
  useEffect(() => {
    const boot = async () => {
    ...
    // Boot immediately - don't wait
    boot();
    
    // Cleanup: Keep instance alive even on unmount for faster subsequent loads
    return () => {
      // Don't teardown the instance to keep it cached
      console.log("Component unmounting but keeping WebContainer alive");
    };
  }, []);
    ...
    <div className='bg-[#161616] min-h-screen flex flex-col text-white'>
      <div className='flex justify-between p-4'>
        ...
        <div className='flex flex-row md:flex-row gap-2 items-center'>
          <button 
            className='bg-slate-800 p-2 md:p-3 text-xs md:text-[16px] rounded-2xl hover:scale-95 hover:bg-slate-700 transition-transform duration-150' 
        ...
        <div className='flex gap-4 items-center'>
          <h1 className='text-xs md:text-lg font-semibold text-slate-400 hover:text-blue-500 hover:scale-105 duration-300'>
            {username}
          </h1>
